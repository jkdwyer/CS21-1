...
import java.util.*;
/**
 * class LinkedDoublesList
 * - contains references to LLDoubleNode objects at head and tail of List.
 *
 * @author Jan Dwyer
 * @version 1.0     09/25/2017
 */
public class LinkedDoublesList {
    private LLDoubleNode head;
    private LLDoubleNode tail;
    private int nodeCounter = 0;

    // no-args constructor.
    public LinkedDoublesList() {
    }   // end constructor.
     ...
     * @param ba String
     */
    public void insertNode(LLDoubleNode node, LLDoubleNode neighbor, String ba) {
        LLDoubleNode neighborLast;
        LLDoubleNode neighborNext;
        if (nodeCounter == 0) {
     ...
     * @param node LLNode
     */
    public void insertNewHead (LLDoubleNode node) {
        if (nodeCounter == 0) {
            head = node;
            tail = node;
        } else {
     ...
     * @return
     */
    public void deleteNode(LLDoubleNode node) {
        LLDoubleNode neighbor;
        LLDoubleNode beforeNode;
        LLDoubleNode afterNode;
        if (!listEmpty()) {
            if (node == head) {
     ...
     * @return position     int
     */
    public int searchByNode(LLDoubleNode node) {
        int position = -1;      // initialize to a non-position value.
        int compareCounter = 1;
        LLDoubleNode compareNode = head;
        boolean match = false;
        ...
        if (!listEmpty()) {
            while ((compareNode != tail) && (!match)){
                if (compareNode == node) {
                    ...
                    LLDoubleNode next = compareNode.getNext();
                    compareNode = next;
                    compareCounter++;
                }   // end if/else.
            }   // end while.
     ...
     * @param pos int
     * @return LLDoubleNode node
     */
    public LLDoubleNode searchByPosition(int pos) {
        LLDoubleNode node = null;
        int counter = 1;
        boolean match = false;
        ...
        if (pos <= nodeCounter) {       // then I can return a node.
            ...
            while ((counter <= nodeCounter) && (!match)) {
                if (pos == counter) {
                    ...
                    LLDoubleNode next = node.getNext();
                    node = next;
                    counter++;
                }   // end if/else.
            }   // end while.
